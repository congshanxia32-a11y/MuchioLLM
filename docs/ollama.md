...
配布用の初期モデルは `qwen3:4b` を推奨します。`qwen3.6` 系の大きいモデルは、一般的なゲーミングPCではメモリ不足や長い待ち時間になりやすいため、最初のモデルには選ばないでください。
...
3. 「あたま(LLM)」カードを開く
4. 表示されたGPU・VRAM・RAMを確認し、「おすすめ」のモデルを選ぶ
5. 未取得なら「ダウンロード」、完了後に「これを使う」を押す
6. 画面下の「ほぞん」を押す

MuchioLLMは、モデル変更を検知すると次回の返答からモデルを温め直します。小さいモデルでは人格設定を優先し、VRCXや日記などの補足情報を減らします。大きいモデルでは状況情報まで渡すため、設定画面の「いまLLMに渡る設定・状況プロンプト」もモデルに応じて変わります。
...
1. まず設定画面のGPU・VRAM・RAM表示を確認する
2. 「おすすめ」のモデルを選ぶ
3. 画面からダウンロードするか、必要なら `ollama pull モデル名` を使う
4. 「これを使う」→「ほぞん」で反映する
